...
export class ShowServiceImpl implements ShowService{
    ...
    async getPagedShows(page:number): Promise<Show[]>{
        let rawShows = await this.storage.findShowsPage(page);
        return this.transformShows(rawShows)
    }

    transformShows(shows: StoredShow[]):Show[]{
    ...
    extractCast(show:StoredShow):Actor[]{
